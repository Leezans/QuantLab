...
_FLAB_UNAVAILABLE = "fLab backend is not implemented yet. UI interface is reserved."


@dataclass
class FLabPlaceholderService:
    _lab_key: str = "futures"
    ...
    def start_task(self, name: str, payload: dict) -> TaskRef:
        return TaskRef(task_id=f"pending-{name}", status="unavailable", detail={"reason": _FLAB_UNAVAILABLE})

    def get_task(self, task_id: str) -> TaskStatus:
        return TaskStatus(task_id=task_id, status="unavailable", detail={"reason": _FLAB_UNAVAILABLE})


# Backward-compatible alias.
FuturesLabService = FLabPlaceholderService
