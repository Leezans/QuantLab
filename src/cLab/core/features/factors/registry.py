...
_global_registry = FactorRegistry() # 模块级全局注册表实例：所有因子注册都会写入这个共享实例，外部查询也通过它访问已注册因子
...
def get_registry() -> FactorRegistry: # 提供一个函数来访问全局注册表实例，外部模块通过这个函数获取已注册的因子信息和计算能力
    return _global_registry # 返回全局因子注册表实例，供外部查询和执行已注册的因子